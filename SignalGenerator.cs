...
/* NOTE:
 * This class is from NAudio, I needed to add functionality to get a single shot sweep (it is 
 * quite possible that it can be done without extending this class).
 */
namespace Bubbles // NAudio.Wave.SampleProviders
{
    /// <summary>
    /// Signal Generator
    ...
    public class SignalGenerator : ISampleProvider
    {
        ...
        private double phi;
private  bool _startSweep;
private bool _makeSweep;
        ...
        public int Read(float[] buffer, int offset, int count)
        {
            ...
            for (int sampleCount = 0; sampleCount < count/waveFormat.Channels; sampleCount++)
            {
                switch (Type)
                {
                    ...
                    case SignalGeneratorType.SweepSingleShot:
                        ...
                        {
                            // Sweep Generator
                            double f2 = Math.Exp(FrequencyLog + (nSample * (FrequencyEndLog - FrequencyLog)) / (SweepLengthSecs * waveFormat.SampleRate));

                            multiple = TwoPi * f2 / waveFormat.SampleRate;
                            phi += multiple;
                            sampleValue = Gain * (Math.Sin(phi));
                            nSample++;
                            if (nSample > SweepLengthSecs * waveFormat.SampleRate)
